...
template <typename T>
__device__ int flipK(curandState* randState, floating_t p = 0.5) {
    return curand_uniform(randState) < p ? 1 : 0;
}

template <typename T> // (min, max]
__device__ floating_t sampleUniform(curandState* randState, floating_t min, floating_t max) {
    return (curand_uniform(randState) * (max - min)) + min;
}

template <typename T> 
__device__ floating_t sampleNormal(curandState* randState, int i, floating_t mean, floating_t std) {
    return (curand_normal(randState) * std) + mean;
}

template <typename T>
__device__ floating_t sampleExponential(curandState* randState, int i, floating_t lambda) {
...
// k = shape, theta = scale
template <typename T>
__device__ floating_t sampleGamma(curandState* randState, int i, floating_t k, floating_t theta) {
...
template <typename T> // [min, max)
floating_t sampleUniform(curandState* randState, int i, floating_t min, floating_t max) {
    return (uniformDist(generatorDists) * (max - min)) + min;
}

template <typename T>
floating_t sampleNormal(curandState* randState, int i, floating_t mean, floating_t std) {
    return (normalDist(generatorDists) * std) + mean;
}

template <typename T>
int flipK(curandState* randState, int i, floating_t p = 0.5) {
    
    return uniformDist(generatorDists) < p ? 1 : 0;
}

template <typename T>
floating_t sampleExponential(curandState* randState, floating_t lambda) {
...
// k = shape, theta = scale
template <typename T>
floating_t sampleGamma(curandState* randState, floating_t k, floating_t theta) {
...
template <typename T>
HOST DEV void sampleDirichlet(curandState* randState, const floating_t* alpha, floating_t* ret, const int n) {
...
template <typename T>
HOST DEV int sampleCategorical(curandState* randState, const floating_t* dist, const int n) {
...
// returns integer [0, n)
template <typename T>
HOST DEV int sampleCategoricalStandard(curandState* randState, const int n) {
