...
BBLOCK_HELPER(clads2GoesUndetectedDelayed, {
  ...
  }, bool, floating_t startTime, gamma_t& lambda0, floating_t factor, floating_t alpha, floating_t sigma, floating_t epsilon, floating_t rho)
...
BBLOCK_HELPER(simBranchDelayed, {
  ...
  }, simBranchRet_t, floating_t startTime, floating_t stopTime, gamma_t& lambda0, floating_t factor, floating_t alpha, floating_t sigma, floating_t epsilon, floating_t rho);
...
BBLOCK(simTree, {
    ...
    simBranchRet_t ret = BBLOCK_CALL(simBranchDelayed, treeP->ages[indexParent], treeP->ages[treeIdx],  PSTATE.lambda_0, factor, PSTATE.alpha, PSTATE.sigma, PSTATE.epsilon, PSTATE.rho);
...
BBLOCK(conditionOnDetection, {
    ...
    for(int i = 0; i < numSamples; i++) {
        bool undetected = BBLOCK_CALL(clads2GoesUndetectedDelayed, treeAge, lambda0, factor, alpha, sigma, epsilon, rho);
        if(! undetected)
            numDetected++;
    }
