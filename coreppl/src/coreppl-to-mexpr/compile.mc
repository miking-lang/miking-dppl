...
lang MExprCompile =
  ...
  | models ->
    mapMapWithKey
      (lam id. lam model.
        ...
        let log = mkPhaseLogState options.debugDumpPhases options.debugPhases in
        endPhaseStats log "enter-compile-models-one" ast;
        ...
        match mapLookup method runtimes.entries with Some entry then
          ...
          endPhaseStats log "transform-model-ast-one" ast;
  ...
  | {ast = modelAst, params = modelParams} ->
    ...
    endPhaseStats log "normalize-term-one" modelAst;
    ...
    endPhaseStats log "symbolize-with-runtime-one" ast;
    ast
