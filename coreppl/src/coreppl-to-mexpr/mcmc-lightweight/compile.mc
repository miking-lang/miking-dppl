...
lang MExprPPLLightweightMCMC =
  ...
  | x ->
    ...
    endPhaseStats log "extract-normal-one" t;
  ...
  | x ->
    ...
    endPhaseStats log "transform-addr-one" t;

    -- Type addressing transform
    let t = mapPre_Expr_Expr (exprTyTransform x.runtime) t in
    endPhaseStats log "empty-ty-transform-one" t;

    -- Transform distributions to MExpr distributions
    let t = mapPre_Expr_Expr (transformTmDist x.dists) t in
    endPhaseStats log "transform-tm-one" t;
    ...
    endPhaseStats log "bind-empty-addr-one" t;
  ...
  | x ->
    ...
    endPhaseStats log "transform-post-aligned-one" t;
