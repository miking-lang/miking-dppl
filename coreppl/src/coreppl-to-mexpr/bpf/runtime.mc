...
let run: all a. (State -> Checkpoint a) -> (ResOption a -> ()) -> () = lam model. lam printResFun.
  match monteCarloArgs () with (particleCount, sweeps) in
  ...
  repeat (lam.
    ...
    recursive let runRec = lam particles.
      if forAll (lam p. match p.checkpoint with End _ then true else false) particles then
        unzip (mapReverse (lam p. (p.weight, match p.checkpoint with End a in Some a)) particles)
      else
