...
lang MExprPPLImportance =
  ...
  | x ->
    ...
    let t = match mapLookup "delayed-sampling" x.extraEnvs with Some env
      then delayedSampling env t
      else t in
...
lang ImportanceCompilerPicker = ImportanceSamplingMethod
  ...
  sem pickRuntime = | Importance x ->
    let extras = if x.dynamicDelay
      then mapSingleton cmpString "delayed-sampling" "delayed-sampling/runtime.mc"
