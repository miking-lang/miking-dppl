...
lang TransformPBN = ConjugatePrior
  ...
  | (RandomVarNode t, MultiplexerNode p)&v ->(if debug then print (join ["createMParameterTDP-muxnode",v2str v.0,"\n"]) else ());
    ...
    let param = seq_ (map (lam i. match i with RandomVarNode v in 
  ...
  | (Some targetPID, None ())  -> 
    ...
    (if debug then print (join ["debug1",v2str v.0,"\n"]) else ());
      match (getPlateId container) with Some pid then 
        ...
        let outerAccId = (nameSym "outerParam") in
  ...
  | PlateNode pl ->
    ...
    ({pbn with g=digraphAddEdge cbInitParam f 0 pbn.g}, f,0)
  | FoldNode fl ->
  ...
  | (Some targetPID, None ())  -> 
      ...
      match deref f.plateId with Some lpid then
        match mapLookup lpid pbn.m with Some (FoldNode gf) in
          ...
          let pbn = {pbn with g=digraphAddEdge (FoldNode f) retBlock 0 pbn.g} in
