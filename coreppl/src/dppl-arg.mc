...
let smcBpfOptions : OptParser ModelOptionsTemp =
  let mk = lam cps. lam resample. lam particles. lam subsample. lam subsampleSize. lam prune. lam dynamicDelay.
  ...
  optMap2 (lam. lam x. x) m (optApply (optApply (optMap5 mk _cps _resample _particles _subsample _subsampleSize) _prune) _dynamicDelay)
