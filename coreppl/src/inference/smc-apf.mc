...
lang APFMethod = InferMethodBase
  ...
  | "APF" ->
    ...
    match getFields info bindings expectedFields
      with [particles, subsample, subsampleSize, resample, cps] in
    APF
    { particles = particles
    , subsample = subsample
    , subsampleSize = subsampleSize
...
let smcApfOptions : OptParser (use APFMethod in InferMethod) =
  use APFMethod in
  let mk = lam particles. lam subsample. lam subsampleSize. lam resample. lam cps. APF
  ...
  let method = optMap5 mk _particles _subsample _subsampleSize _resample _cps in
