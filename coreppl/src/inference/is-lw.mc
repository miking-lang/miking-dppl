...
lang ImportanceSamplingMethod = InferMethodBase
  ...
  | "Importance" ->
    ...
    match getFields info bindings expectedFields with [particles, earlyStop, cps, dynamicDelay] in
    Importance
    { particles = particles
    , earlyStop = earlyStop
...
let isLwOptions : OptParser (use ImportanceSamplingMethod in InferMethod) =
  ...
  let mk = lam particles. lam earlyStop. lam cps. lam dynamicDelay. Importance
    { particles = int_ particles
    , earlyStop = bool_ earlyStop
    , cps = cps
    , dynamicDelay = dynamicDelay
    } in
  let method = optMap4 mk _particles _earlyStop _cps _dynamicDelay in
