...
lang LightweightMCMCMethod = InferMethodBase
  ...
  | LightweightMCMC t ->
    ...
    let pigeonsIID = bool2string t.pigeonsIID in
    ( env
    , join
      [ "(LightweightMCMC "
  ...
  | "LightweightMCMC" ->
    let expectedFields =
      ...
      , ("pigeonsIID", bool_ _pigeonsIIDDefault)
    ...
    with [continue, keepSample, debug, globalProb, driftKernel, pigeons, pigeonsIID, driftScale, cps, align, debugAlignment] in
    LightweightMCMC
    { continue = continue
    , keepSample = keepSample
    , globalProb = globalProb
    ...
    , pigeonsIID = _exprAsBoolExn pigeonsIID
...
let mcmcLightweightOptions : OptParser (use LightweightMCMCMethod in InferMethod) =
  ...
  let mk = lam particles. lam globalProb. lam driftKernel. lam driftScale. lam cps. lam align. lam debugAlignment. lam pigeons. lam pigeonsIID. LightweightMCMC
    { keepSample = ulam_ "" true_
    , continue = utuple_
      [ int_ particles
      , ulam_ "remaining" (ulam_ ""
  ...
  let method = optApply (optApply (optApply (optApply (optMap5 mk _particles _mcmcLightweightGlobalProb _driftKernel _driftScale _cps) _align) _debugAlignment) _pigeons) _pigeonsIID in
