...
let d =
  infer (LightweightMCMC { continue = (lam. 100000, lam x. lam. lam. (subi x 1, neqi x 0)), globalProb = 0.1 }) model1
in
utest expectation d with expected using eqfApprox 1e-1 in
...
let d =
  infer (LightweightMCMC { continue = (lam. 1, lam x. lam. lam. (subi x 1, neqi x 0)), globalProb = 0.1 }) model2
in
utest expectation d with expected using eqfApprox 1e-1 in
