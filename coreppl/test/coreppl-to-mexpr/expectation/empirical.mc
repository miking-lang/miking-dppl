...
let d =
  infer (LightweightMCMC { continue = (100000, lam x. lam. (subi x 1, geqi x 0)), globalProb = 0.1 }) model1
in
utest expectation d with expected using eqfApprox 1e-1 in
...
let d =
  infer (LightweightMCMC { continue = (1, lam x. lam. (subi x 1, geqi x 0)), globalProb = 0.1 }) model2
in
utest expectation d with expected in
