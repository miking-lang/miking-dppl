...
let model : () -> () = lam.
  ...
  assert "scalar derivative" (eqf (diff f x 1.) (df x 1.));
  ...
  let v = [1., 0.] in
  assert "partial derivatives 1.1" (eqSeq eqf (diff g xs v) (dg xs v));

  -- Second partial derivative
  let v = [0., 1.] in
  assert "partial derivatives 1.2" (eqSeq eqf (diff g xs v) (dg xs v));

  -- Jacobian-vector-product
  let v = [2., 3.] in
  assert "partial derivatives 1.JvP" (eqSeq eqf (diff g xs v) (dg xs v));
  ...
  assert "partial derivatives 2.1"
    (eqSeq eqf (diff h x v).a (dh x v).a);
  ...
  -- assert "Differentiating through assume 3.1" (eqf (diff1 f 2.) 2.); -- Should result in a runtime error
